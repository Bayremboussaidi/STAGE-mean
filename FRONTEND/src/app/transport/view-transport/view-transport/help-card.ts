...
@Component({
  ...
  template: `
    <h4
      class="text-[20px] text-dark dark:text-white/[.87] font-medium mb-[25px]"
    >
      Need to help for adding new contact
    </h4>
    <div nz-row nzGutter="25">
      ...
      >
        <div>
          <h4
            class="text-[14px] text-light dark:text-white/60 mb-[12px] font-normal"
          >
            {{ item.authorArea.slug }}:
          </h4>
          <div class="flex items-center mb-[30px] gap-[15px]">
            <ng-container
              *ngIf="item.authorArea.avatar.length === 0; else showOptions"
            >
            </ng-container>
            <ng-template #showOptions>
              <img
                class="w-[30px] h-[30px] rounded-full"
                [src]="item.authorArea.avatar"
                alt="author image"
              />
            </ng-template>
            <span
              class="text-[14px] font-medium text-theme-gray dark:text-white/60 capitalize"
              >{{ item.authorArea.name }}</span
            >
          </div>
          ...
          <ng-template #showPriority>
            <h4
              class="text-theme-gray dark:text-white/60 mb-[12px] font-medium"
            >
              {{ item.priority }}
            </h4>
          </ng-template>
        ...
        <ng-container
          *ngIf="item.priorityOptions.length === 0; else showSelect"
        >
        </ng-container>
        <ng-template #showSelect>
          ...
          >
            <nz-option
              *ngFor="let option of item.priorityOptions"
    ...
    <div class="">
      <h4
        class="text-[20px] font-semibold leading-1 text-theme-gray dark:text-white/60 mb-[12px]"
      >
        Overview :
      </h4>
      <p class="text-theme-gray dark:text-white/60 text-[16px] font-normal">
        There are have a many variations of passages of Lorem Ipsum available,
        but the randomised words which don't look even slightly believable. If
        you are going to sdsss embarrassing hidden in the middle of text. All
