        ...
        <ul class="p-0">
          <li
            *ngFor="let item of files"
            class="p-0 last:mb-0 mb-[20px] border-none flex items-center justify-between"
          >
            <div class="flex items-center gap-[15px]">
              <div class="border-none p-0 items-center">
                <img
                  class="max-w-[40px] rounded-[5px]"
                  [src]="item.icon"
                  alt="hexadash Team"
                />
              </div>
              <div>
                <a
                  class="text-theme-gray text-[15px] dark:text-white/60 hover:text-primary font-medium leading-[19px]"
                  href="#"
                >
                  {{ item.name }}
                </a>
                <div
                  class="text-[15px] text-light dark:text-white/60 leading-1"
                >
                  {{ item.size }}
                </div>
              </div>
            ...
            <button
              nz-button
              class="flex items-center justify-center border-none shadow-none bg-transparent"
            >
              <span
                class="text-[18px] text-light dark:text-white/60 dark:hover:text-primary hover:text-primary"
                nz-icon
                [nzType]="item.typeStatus"
                nzTheme="outline"
              ></span>
            </button>
          </li>
...
export class FileAttachmentComponent implements OnInit {
  ...
  ngOnInit() {
    this.http
      .get<any>('assets/data/pages/file-attach.json')
      .subscribe((data) => {
