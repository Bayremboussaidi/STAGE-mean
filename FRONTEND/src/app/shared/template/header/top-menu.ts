...
@Component({
  ...
  template: `
    <div
      class="hexadash-top-menu position-relative h-full items-center max-lg:hidden"
    >
            ...
            <ul class="ps-[30px] flex items-center flex-wrap">
              <ng-container *ngFor="let route of menuItems">
                <ng-container *ngIf="route.submenu.length > 0; else menuLink">
                  <li
                    class="has-subMenu text-[14px] flex items-center font-medium 3xl:py-[25px] max-3xl:py-[6px] text-theme-gray dark:text-white/[.87] relative gap-[6px]"
                    [ngClass]="{
                      'no-link-submenu': hasBlankPath(route.submenu)
                    }"
                  >
                    {{ route.title }}
                    <span
                    ...
                    <ul class="subMenu">
                      <ng-container *ngFor="let subroute of route.submenu">
                        <ng-container
                          *ngIf="subroute.submenu.length > 0; else subMenuLink"
                        >
                          <li
                            [ngClass]="{
                              'has-blank-path': hasBlankPath(subroute.submenu)
                            }"
                          >
                            <a
                            ...
                            <ul class="subMenu">
                              <ng-container
                                *ngFor="let subSubroute of subroute.submenu"
                              >
                                <li>
                                  <a
                                    [routerLink]="subSubroute.path"
                                    routerLinkActive="active"
                                  >
