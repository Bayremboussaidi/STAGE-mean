...
@Component({
  ...
  template: `
    <div
      class="bg-regularBG dark:bg-white/10 px-[25px] pb-[25px] pt-[15px] max-sm:px-[15px] rounded-[10px]"
    >
      <div class="flex items-center justify-between h-[50px]">
        <h2 class="text-light dark:text-white/60 text-[15px] font-medium">
          Notifications
        </h2>
        <button
          nz-button
          class="font-normal text-info text-[13px] border-none outline-none shadow-none bg-transparent"
      ...
      <div
        class="bg-white dark:bg-white/10 shadow-[0_5px_20px_rgba(173,181,217,0.05)] rounded-[10px]"
      >
        <nav>
          <ul class="p-0 m-0">
            <li
              class="flex items-center justify-between mb-0 px-[25px] py-[20px] border-b border-regular last:border-none dark:border-white/10 [&>button]:shadow-none gap-[15px]"
              *ngFor="let item of notification"
            >
              <div>
                <h4
                  class="mb-0.5 text-body dark:text-white/60 text-sm font-medium"
                >
                  {{ item.title }}
                </h4>
                <p class="mb-0 text-light dark:text-white/60">
                  {{ item.description }}
                </p>
              </div>
              <div>
...
export class NotificationComponent {
  ...
  toggleAllSwitches() {
    const allSwitchStatus = this.notification.every(
      (item) => item.switchStatus
    );
