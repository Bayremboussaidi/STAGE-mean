  ...
  template: `
    <form class="max-w-full" nz-form [formGroup]="myGroup">
      <nz-form-control class="mb-[20px]" *ngFor="let item of socialItems">
        ...
        >
        <nz-input-group
          class="w-full rounded-6 border-normal border-1 text-[15px] dark:bg-white/10 dark:border-white/10  outline-none placeholder:text-[#A0A0A0] text-theme-gray dark:text-white/60 inline-flex items-center"
        >
          <button
            class="h-[48px] w-[45px] rounded-6 bg-{{
              item.name
            }} shadow-none outline-none border-{{
              item.name
            }} inline-flex items-center justify-center ltr:ml-[1px] rtl:mr-[1px]"
            nz-button
          >
            <svg-icon
              class="[&>svg]:w-[22px] [&>svg]:h-[22px] text-white"
              src="assets/images/svg/unicons-line/{{ item.iconName }}.svg"
            ></svg-icon>
          </button>
          <input
            class="bg-transparent border-none outline-none shadow-none min-h-[50px] placeholder:text-[14px] placeholder:text-theme-gray dark:placeholder:text-white/60 px-[20px]"
            type="{{ item.type }}"
            nz-input
